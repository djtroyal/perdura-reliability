  ...
  for (const { gp, st } of folio) {
    ...
    if (ftNodes?.length) {
      out.push({
        ...
        group: gp, label: 'Fault Tree Diagram', type: 'plot',
        getData: () => {
          const GATE_COLORS: Record<string, string> = {
            and: '#4f46e5', or: '#ea580c', vote: '#9333ea', pand: '#0d9488',
            xor: '#e11d48', not: '#475569', transfer: '#0891b2', basic: '#6b7280',
          }
          const nodeX: number[] = [], nodeY: number[] = [], nodeText: string[] = [], nodeColor: string[] = []
          const posMap = new Map<string, { x: number; y: number }>()
          for (const n of ftNodes) {
            const x = n.position?.x ?? 0, y = -(n.position?.y ?? 0)
            posMap.set(n.id, { x, y })
            nodeX.push(x); nodeY.push(y)
            const label = String(n.data?.label ?? n.id)
            const prob = n.data?.computedP != null ? `<br>P=${Number(n.data.computedP).toExponential(2)}` : ''
            nodeText.push(`${(n.type ?? 'basic').toUpperCase()}: ${label}${prob}`)
            nodeColor.push(GATE_COLORS[n.type ?? 'basic'] ?? '#6b7280')
          }
          const edgeX: (number | null)[] = [], edgeY: (number | null)[] = []
          for (const e of (ftEdges ?? [])) {
            const src = posMap.get(e.source), tgt = posMap.get(e.target)
            if (src && tgt) { edgeX.push(src.x, tgt.x, null); edgeY.push(src.y, tgt.y, null) }
          }
          return {
            plotData: [
              { x: edgeX, y: edgeY, mode: 'lines', type: 'scatter', line: { color: '#9ca3af', width: 1.5 }, hoverinfo: 'skip', showlegend: false },
              { x: nodeX, y: nodeY, mode: 'markers+text', type: 'scatter', text: nodeText, textposition: 'top center', textfont: { size: 8 },
                marker: { color: nodeColor, size: 18, line: { color: '#1e293b', width: 1 } }, hovertemplate: '%{text}<extra></extra>', showlegend: false },
            ],
            plotLayout: { ...BASE, xaxis: { showgrid: false, zeroline: false, showticklabels: false }, yaxis: { showgrid: false, zeroline: false, showticklabels: false }, title: { text: 'Fault Tree Topology' }, height: 500 },
          }
        },
      })
    }
    ...
    if (r.methods && Object.keys(r.methods).length > 0) {
      out.push({
        id: mkId('fta'), module: 'faultTree', moduleLabel: 'Fault Tree Analysis',
        group: gp, label: 'Method Comparison', type: 'table',
        getData: () => ({
          tableHeaders: ['Method', 'P(TOP)'],
          tableRows: Object.entries(r.methods!).map(([m, v]) => [m, v != null ? fmt(v as number) : '—']),
        }),
      })
    }
  }
  ...
  for (const { gp, st } of folio) {
    ...
    if (rbdNodes?.length) {
      out.push({
        ...
        group: gp, label: 'Block Diagram', type: 'plot',
        getData: () => {
          const NODE_COLORS: Record<string, string> = { source: '#10b981', sink: '#ef4444', component: '#3b82f6' }
          const nodeX: number[] = [], nodeY: number[] = [], nodeText: string[] = [], nodeColor: string[] = [], nodeSymbol: string[] = []
          const posMap = new Map<string, { x: number; y: number }>()
          for (const n of rbdNodes) {
            const x = n.position?.x ?? 0, y = -(n.position?.y ?? 0)
            posMap.set(n.id, { x, y })
            nodeX.push(x); nodeY.push(y)
            const label = String(n.data?.label ?? n.id)
            ...
            nodeSymbol.push(n.type === 'source' || n.type === 'sink' ? 'diamond' : 'square')
          }
          const edgeX: (number | null)[] = [], edgeY: (number | null)[] = []
          for (const e of (rbdEdges ?? [])) {
            const src = posMap.get(e.source), tgt = posMap.get(e.target)
            if (src && tgt) { edgeX.push(src.x, tgt.x, null); edgeY.push(src.y, tgt.y, null) }
          }
          return {
            plotData: [
              { x: edgeX, y: edgeY, mode: 'lines', type: 'scatter', line: { color: '#9ca3af', width: 2 }, hoverinfo: 'skip', showlegend: false },
              { x: nodeX, y: nodeY, mode: 'markers+text', type: 'scatter', text: nodeText, textposition: 'top center', textfont: { size: 9 },
                marker: { color: nodeColor, size: 22, symbol: nodeSymbol, line: { color: '#1e293b', width: 1 } }, hovertemplate: '%{text}<extra></extra>', showlegend: false },
            ],
            plotLayout: { ...BASE, xaxis: { showgrid: false, zeroline: false, showticklabels: false }, yaxis: { showgrid: false, zeroline: false, showticklabels: false }, title: { text: 'Reliability Block Diagram' }, height: 400 },
          }
        },
      })
    }
...
function extractMarkov(modules: Record<string, unknown>, out: AssetDescriptor[]) {
  const s = modules['markov'] as { result?: MarkovResponse | null; states?: Any[]; transitions?: Any[] } | null
  const r = s?.result
  if (!r) return
  const MOD = 'markov', ML = 'Markov Analysis', GP = 'Markov'

  // State diagram (circular layout with annotation arrows)
  if (r.states?.length) {
    out.push({
      id: mkId('mkv'), module: MOD, moduleLabel: ML,
      group: GP, label: 'State Diagram', type: 'plot',
      getData: () => {
        const n = r.states.length
        const STATE_COLORS: Record<string, string> = { operational: '#10b981', degraded: '#f59e0b', failed: '#ef4444' }
        const cx: number[] = [], cy: number[] = [], ct: string[] = [], cc: string[] = []
        const posMap = new Map<string, { x: number; y: number }>()
        r.states.forEach((s, i) => {
          const angle = (2 * Math.PI * i) / n - Math.PI / 2
          const x = Math.cos(angle) * 3, y = Math.sin(angle) * 3
          posMap.set(s.id, { x, y })
          cx.push(x); cy.push(y)
          const ss = r.steady_state?.[s.id ?? s.name]
          ct.push(`${s.name}${ss != null ? `<br>π=${fmt(ss)}` : ''}`)
          cc.push(STATE_COLORS[s.type] ?? '#6b7280')
        })
        const annotations = r.transitions.map(t => {
          const src = posMap.get(t.from), tgt = posMap.get(t.to)
          if (!src || !tgt) return null
          const dx = tgt.x - src.x, dy = tgt.y - src.y
          const len = Math.sqrt(dx * dx + dy * dy) || 1
          const shrink = 0.35
          return {
            x: tgt.x - (dx / len) * shrink, y: tgt.y - (dy / len) * shrink,
            ax: src.x + (dx / len) * shrink, ay: src.y + (dy / len) * shrink,
            xref: 'x', yref: 'y', axref: 'x', ayref: 'y',
            showarrow: true, arrowhead: 2, arrowsize: 1.2, arrowwidth: 1.5, arrowcolor: '#64748b',
            text: t.label || String(t.rate), font: { size: 8, color: '#475569' },
          }
        }).filter(Boolean)
        return {
          plotData: [{
            x: cx, y: cy, mode: 'markers+text', type: 'scatter',
            text: ct, textposition: 'top center', textfont: { size: 10 },
            marker: { color: cc, size: 30, line: { color: '#1e293b', width: 1.5 } },
            hovertemplate: '%{text}<extra></extra>', showlegend: false,
          }],
          plotLayout: {
            ...BASE, annotations,
            xaxis: { showgrid: false, zeroline: false, showticklabels: false, range: [-5, 5] },
            yaxis: { showgrid: false, zeroline: false, showticklabels: false, scaleanchor: 'x', range: [-5, 5] },
            title: { text: 'Markov State Diagram' }, height: 500,
          },
        }
      },
    })
  ...
  if (r.time_dependent?.length) {
    const stateNames = Object.keys(r.time_dependent[0].state_probs)
    out.push({
      id: mkId('mkv'), module: MOD, moduleLabel: ML,
      group: GP, label: 'State Probabilities', type: 'plot',
      getData: () => {
        const times = r.time_dependent!.map(e => e.time)
        return {
          plotData: stateNames.map((name, i) => ({
            x: times, y: r.time_dependent!.map(e => e.state_probs[name]),
            mode: 'lines', name, line: { color: COLORS[i % COLORS.length], width: 2 },
          })),
          plotLayout: { ...BASE, xaxis: { title: { text: 'Time' }, gridcolor: GREY }, yaxis: { title: { text: 'P(state)' }, range: [0, 1], gridcolor: GREY }, title: { text: 'State Probabilities vs Time' }, showlegend: true },
        }
      },
    })

    // Availability / Reliability plot
    out.push({
      id: mkId('mkv'), module: MOD, moduleLabel: ML,
      group: GP, label: 'Availability & Reliability', type: 'plot',
      getData: () => {
        const times = r.time_dependent!.map(e => e.time)
        return {
          plotData: [
            { x: times, y: r.time_dependent!.map(e => e.availability), mode: 'lines', name: 'A(t)', line: { color: '#10b981', width: 2 } },
            { x: times, y: r.time_dependent!.map(e => e.reliability), mode: 'lines', name: 'R(t)', line: { color: '#3b82f6', width: 2, dash: 'dash' } },
            { x: times, y: r.time_dependent!.map(e => e.unavailability), mode: 'lines', name: 'U(t)', line: { color: '#ef4444', width: 1.5, dash: 'dot' } },
          ],
          plotLayout: { ...BASE, xaxis: { title: { text: 'Time' }, gridcolor: GREY }, yaxis: { title: { text: 'Probability' }, range: [0, 1.02], gridcolor: GREY }, title: { text: 'Availability & Reliability' }, showlegend: true },
        }
      },
    })
  }
  ...
  if (sp) {
    out.push({
      id: mkId('mkv'), module: MOD, moduleLabel: ML,
      group: GP, label: 'System Parameters', type: 'metrics',
      getData: () => ({
        metrics: [
          { label: 'A_ss (Steady-State Availability)', value: fmt(sp.availability_ss) },
          { label: 'U_ss (Steady-State Unavailability)', value: fmt(sp.unavailability_ss) },
  ...
  if (r.steady_state && Object.keys(r.steady_state).length) {
    out.push({
      id: mkId('mkv'), module: MOD, moduleLabel: ML,
      group: GP, label: 'Steady-State Probabilities', type: 'table',
      getData: () => ({
        tableHeaders: ['State', 'π (Steady-State Probability)'],
        tableRows: Object.entries(r.steady_state!).map(([name, prob]) => [name, fmt(prob)]),
      }),
    })
  }

  // Transition rate matrix table
  if (r.transition_matrix?.length && r.states?.length) {
    out.push({
      id: mkId('mkv'), module: MOD, moduleLabel: ML,
      group: GP, label: 'Transition Matrix (Q)', type: 'table',
      getData: () => {
        const names = r.states.map(s => s.name)
        return {
          tableHeaders: ['From \\ To', ...names],
          tableRows: r.transition_matrix.map((row, i) => [names[i], ...row.map(v => fmt(v))]),
        }
      },
    })
  }
