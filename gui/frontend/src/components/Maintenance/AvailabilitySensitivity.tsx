...
export default function AvailabilitySensitivity() {
  return <InfluenceScope className="flex h-full w-full"><AvailabilitySensitivityContent /></InfluenceScope>
}

function AvailabilitySensitivityContent() {
  ...
  const controls = (
    <>
      ...
      <InfluenceSource influence="availability.mtbf" className="-m-1 p-1">
      ...
      <InfluenceSource influence="availability.mttr" className="-m-1 p-1">
      ...
      <InfluenceSource influence="availability.admin" className="-m-1 p-1">
      ...
      <InfluenceSource influence="availability.logistics" className="-m-1 p-1">
      ...
      <InfluenceSource influence="availability.swing" className="-m-1 p-1">
      ...
      <InfluenceSource influence="availability.target" className="-m-1 p-1">
  ...
  const results = res && (
    <div ref={resultsRef}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-5">
        <InfluenceTarget influences={['availability.mtbf', 'availability.mttr', 'availability.admin', 'availability.logistics']}><Card label="Operational availability" value={(res.baseline_availability * 100).toFixed(3) + '%'} accent /></InfluenceTarget>
        <InfluenceTarget influences={['availability.mttr', 'availability.admin', 'availability.logistics']}><Card label={`Mean down time (${units})`} value={fmtNum(res.mean_down_time)} /></InfluenceTarget>
        <InfluenceTarget influences="availability.swing"><Card label="Most sensitive driver" value={res.tornado[0]?.driver ?? '—'} tip="The input whose ±swing moves availability the most." /></InfluenceTarget>
      </div>
      {res.solve && (
        <InfluenceTarget influences="availability.target" className={`mb-5 p-3 rounded-lg border text-xs ${res.solve.achievable ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-amber-50 border-amber-200 text-amber-800'}`}>
      ...
      <InfluenceTarget influences={['availability.mtbf', 'availability.mttr', 'availability.admin', 'availability.logistics', 'availability.swing']} className="bg-white border border-gray-200 rounded-lg" style={{ height: 360 }}>
        <Plot
          data={[
            { type: 'bar', orientation: 'h', x: res.tornado.map(d => d.range), y: res.tornado.map(d => d.driver), marker: {
              color: res.tornado.map(d => driverInfluence(d.driver) === active ? '#2563eb' : '#0ea5e9'),
              line: { color: res.tornado.map(d => driverInfluence(d.driver) === active ? '#1e3a8a' : '#0ea5e9'), width: res.tornado.map(d => driverInfluence(d.driver) === active ? 2 : 0) },
            } } as Plotly.Data,
          ]}
          layout={{
...
function driverInfluence(driver: string): string | null {
  const value = driver.toLowerCase()
  if (value.includes('mtbf')) return 'availability.mtbf'
  if (value.includes('mttr')) return 'availability.mttr'
  if (value.includes('admin')) return 'availability.admin'
  if (value.includes('logistic')) return 'availability.logistics'
  return null
}
