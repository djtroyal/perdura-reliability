...
# These changes alter computed semantics. Input-only demos may adopt the new
# revision, but historical outputs must be regenerated by their real engines.
INPUT_ONLY_ENGINE_UPGRADES = {"warranty": 3, "reliabilityTestingTools": 2}
...
def has_computed_output(value: Any) -> bool:
    if isinstance(value, list):
        return any(has_computed_output(item) for item in value)
    if not isinstance(value, dict):
        return False
    for key, child in value.items():
        if (key in {"run", "finalized", "assets"} or re.search(r"results?$", key, re.I)):
            if child is not None and child != {} and child != []:
                return True
        if has_computed_output(child):
            return True
    return False


def demo_engine_revisions(demo: dict[str, Any]) -> dict[str, int]:
    previous = demo.get("engineRevisions", {})
    revisions = {
        key: max(previous.get(key, 1), 2 if key in {
            "fmea", "systemDefinition", "lifeData", "markov",
        } else 1)
        for key in demo["modules"]
    }
    for key, revision in INPUT_ONLY_ENGINE_UPGRADES.items():
        if key not in demo["modules"]:
            continue
        old_revision = previous.get(key, 1)
        if old_revision < revision and has_computed_output(demo["modules"][key]):
            raise ValueError(
                f"Cannot relabel {key} computed outputs from revision {old_revision} "
                f"as revision {revision}; regenerate them with the real API or "
                "explicitly remove the old outputs before refreshing input-only demos."
            )
        revisions[key] = max(old_revision, revision)
    return revisions
...
def expected_demo() -> dict[str, Any]:
    ...
    demo.update({
        ...
        "engineRevisions": demo_engine_revisions(demo),
